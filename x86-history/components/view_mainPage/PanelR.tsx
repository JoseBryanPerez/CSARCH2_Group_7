...
export default function Panel({image,headText,bodyText,link}:{image?:string,headText?:string,bodyText?:string,link?:string}){
    return(
      ...
      <Link href={link||"/"} className="md:hidden w-24 h-24 rounded-full overflow-hidden">
        <img src={image} alt={image} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }}/> 
      </Link>
      <Link href="/panels/NASM"  className="hidden md:block  w-[10vw] h-[10vw] overflow-hidden shadow-sm shrink-0 rounded-full">
           <img src={image} alt={image}   style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }}/>
      </Link>
