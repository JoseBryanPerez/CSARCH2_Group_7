...
export default function Panel({image,headText,bodyText,link}:{image?:string,headText?:string,bodyText?:string,link?:string}){
    return(
        <div className="h-auto w-[90vw] md:gap-20 gap-10 p-10 flex flex-wrap items-center justify-center bg-[#111844] rounded-[50px] shadow-2xl">
            ...
            <div className="w-[60vw] h-auto gap-5 p-2 flex flex-col items-center justify-end rounded-lg shadow-md">
                <h2 className="text-[clamp(1.50rem,0.5vw+0.8rem,1.75rem)] font-[Perpetua] text-justify text-white mt-4">{bodyText}</h2>
                
                <div className="w-full flex md:justify-end items-center justify-center">
                <Link href={link||"/"}>
                    <button className="py-2 px-10 text-[clamp(1.50rem,0.5vw+0.8rem,1.75rem)] text-white font-[Perpetua] hover:bg-[#7288AE] rounded">
                        learn more
                    </button>  
                </Link>     
                </div>         
